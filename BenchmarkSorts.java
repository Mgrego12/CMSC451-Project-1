...
import java.util.*;
/*
Matt Gregorek Project One
June 13, 2023
CMSC451 BenchmarkSorts.java Class should use both sorting Algorithms and
perform some critical operation to count for each algorithm, must measure
runtime in nano-seconds. Should randomly generate data to pass to sorting methods.
40 data sets , size of the data set and average result of those 40 runs.
 */
public class BenchmarkSorts extends AbstractSort {

    private static final int DATA_SET_COUNT = 40;
    private static final int[] DATA_SET_SIZES = {100, 200, 300, 400, 500,
    ...
    //private AbstractSort checkSorted;
   ...
   // Displays the Menu for the User to start program, and choose to select file output from directory.
    public void displayMenu() {
        System.out.println("Welcome to Project One.");
        System.out.println("Proceed to tests? 1 for Yes, 2 for No.");
        int ans = scanner.nextInt();
        if (ans == 1) {
            BenchmarkSorts.CriticalOps.JVMWarmUp();
            runSortingMethods();
            outputFile();
            System.out.println("Press 1 again to select which sorted output to view.");
            ans = scanner.nextInt();
            if (ans == 1) {
                selectFile();
                generateReport();
